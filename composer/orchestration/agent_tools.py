...
def make_agent_tools(worker_names, llm, on_event=None, depth=0):
    return [_make_one(name, llm, on_event, depth) for name in worker_names]


def _make_one(name, llm, on_event, depth):
    ...
    def fn(inp):
        ...
        if subs and depth + 1 < MAX_DELEGATION_DEPTH:
            tools += make_agent_tools(subs, llm, on_event, depth + 1)
